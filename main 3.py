...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Automate PR creation for LLM-generated tests."
    )
    parser.add_argument(
        "--repo",
        required=True,
    ...
    parser.add_argument(
        "--base-branch", default="main", help="Base branch for the pull request"
    )
    parser.add_argument("--author", required=True, help="Author name")
    parser.add_argument("--groq-api-key", required=True, help="Groq API key")
    parser.add_argument(
        "--groq-model", default="mixtral-8x7b-32768", help="Groq model to use"
    )
    parser.add_argument(
        "--diff", required=True, help="Git diff text to send to the LLM"
    )
...
def generate_pr_title_desc(author_name, groq_api_key, model, diff_text):
    ...
    try:
        ...
        description = (
            lines[1].replace("Description:", "").strip() if len(lines) > 1 else ""
        )
...
if __name__ == "__main__":
    ...
    pr_title, pr_desc = generate_pr_title_desc(
        author_name, groq_api_key, groq_model, diff
    )
    create_pull_request(
        owner, repo_name, github_token, base_branch, branch_name, pr_title, pr_desc
    )
